...
sidebar_position: 3
title: Configuration
description: Configure core features for your ticket system.
---
...
| Command | Description |
| --- | --- |
| ` /ticket admin-open ` | *Only available if “Allow staff to open tickets on behalf of a user?” is [enabled](#main-configuration-configuration).*<br/>Allows team members to create new tickets with specific users. Restrictions like multiple open tickets or opening hours are bypassed. |
| ` /ticket close ` | Allows team members (and users, if [configured](#ticket-closing-configuration)) to close the ticket in the current channel. |
| ` /ticket form ` | Sends a specific [form](/docs/support-bot/general/forms) in the ticket. |
| ` /ticket open ` | *Only available if “Allow users to open tickets via command?” is [enabled](#main-configuration-configuration).*<br/>Opens a new ticket (with a specific topic) on your Discord server. |

### Configuration {#main-configuration-configuration}

| Field | Description |
| --- | --- |
| Enable ticket system? | Enables or disables the ticket system for your Discord server. |
...
| Allow users to open tickets via command? | If enabled, your users can create tickets not only via buttons and dropdowns, but also with [`/ticket open`](#main-configuration-commands). |
| Allow staff to open tickets on behalf of a user? | If enabled, team members can create a ticket with a user using [`/ticket admin-open`](#main-configuration-commands). Restrictions like multiple open tickets or opening hours are bypassed. |
| Category for new tickets | New tickets are created in this category by default. You can set different categories for different ticket topics on the [Ticket topics](https://scnx.app/glink?page=support-system/ticket-system/ticket-topics) page in your dashboard. More info [here](/docs/support-bot/ticket-system/ticket-topics). |
| Team member roles | These roles get access to tickets by default and are mentioned when a new ticket is created. You can set different roles for different ticket topics on the [Ticket topics](https://scnx.app/glink?page=support-system/ticket-system/ticket-topics) page in your dashboard. More info [here](/docs/support-bot/ticket-system/ticket-topics). |
...
| Ticket message | Your bot will send this message by default when a user creates a new ticket. You can set different messages for different ticket topics on the [Ticket topics](https://scnx.app/glink?page=support-system/ticket-system/ticket-topics) page in your dashboard. More info [here](/docs/support-bot/ticket-system/ticket-topics). |
| Allow only one ticket per user (global) | If enabled, a user can have at most one ticket open at a time. You can set different ticket limits per topic on the [Ticket topics](https://scnx.app/glink?page=support-system/ticket-system/ticket-topics) page in your dashboard. More info [here](/docs/support-bot/ticket-system/ticket-topics). |
| AI summary of ticket transcripts | If enabled, after closing the ticket your bot will automatically send an AI-generated summary of the ticket to the configured log channel. Note that this uses your server’s [AI credit](https://faq.scnx.app/ki-auf-scnx/) and certain data is shared with “modmail.net” (an SCNX product). You can find our privacy policy [here](https://scootk.it/scnx-privacy). |
| Allow team members to rename tickets? | If enabled, team members can rename tickets with [`/ticket rename`](#main-configuration-commands). Note: after a manual rename, the bot can no longer automatically change the ticket name when transferring it (if enabled in [ticket topics](/docs/support-bot/ticket-system/ticket-topics)). |
| Show rename button under the ticket message? | *Only available if “Allow team members to rename tickets?” is enabled.*<br/>If enabled, a button is added under the ticket message so team members can rename the ticket faster. Otherwise they must use [`/ticket rename`](#main-configuration-commands). |
| Rename button text | *Only available if “Allow team members to rename tickets?” and “Show rename button under the ticket message?” are enabled.*<br/>This text is shown on the button under the ticket message. |
...
* Visit the [Ticket opening messages](https://scnx.app/glink?page=support-system/ticket-system/configuration?show=ticket-opening-messages) section in your dashboard.
...
| Field | Description |
| --- | --- |
| Channel to send the message to | Your configured message will be sent to this channel. |
| Message | Your bot will send this message into the selected channel. |
| Show only specific ticket topics? | If enabled, your bot will only use selected [ticket topics](/docs/support-bot/ticket-system/ticket-topics). If you want to use buttons, you can select at most one topic here. |
...
| Command | Description |
| --- | --- |
| ` /ticket close [force:<Boolean>] ` | *Only available if “Allow team members to close tickets without confirmation?” is [enabled](#ticket-closing-configuration).*<br/>Allows team members to close tickets even if a close request is open by setting the `force` option to `True`. |
| ` /ticket close-schedule ` | *Only available if “Can team members manually schedule when tickets should be closed?” is [enabled](#ticket-closing-configuration).*<br/>Allows team members to schedule closing tickets instead of closing them immediately. |

### Configuration {#ticket-closing-configuration}

| Field | Description |
| --- | --- |
| Show ticket close button in the ticket message? | If enabled, a button is added to the ticket message that team members (or users, if enabled) can use to close the ticket. Otherwise they must use [`/ticket close`](#ticket-closing-commands). |
...
| Send a DM confirmation when a ticket is closed? | If enabled, your bot will send a DM to the user after the ticket is closed. |
...
| Attach transcript to the ticket close DM? | *Only available if “Send a DM confirmation when a ticket is closed?” is enabled.*<br/>If enabled, the bot will attach a transcript of the ticket to the DM. |
...
| Timeout for the close request | *Only available if “Require user confirmation before the ticket is closed?” is enabled.*<br/>Within this period, the user must respond to the close request before the ticket is closed automatically. Learn how to set durations in SCNX [here](/docs/custom-bot/additional-features/#durations). |
| Allow team members to close tickets without confirmation? | *Only available if “Require user confirmation before the ticket is closed?” is enabled.*<br/>If enabled, team members can close a ticket despite an open close request using [`/ticket close [force:<Boolean>]`](#ticket-closing-commands). |
| Can team members manually schedule when tickets should be closed? | If enabled, team members can schedule closing tickets with [`/ticket close-schedule`](#ticket-closing-commands). |
...
| Field | Description |
| --- | --- |
| Allow additional ticket members? | Enables or disables the feature for your ticket system. |
| Show “add additional users” button in the ticket message | If enabled, a button is added to the ticket message to add ticket members. Otherwise, users must use `/ticket users add`. |
...
| Field | Description |
| --- | --- |
| Automatically close ticket when a user leaves the server | If enabled, your bot will automatically close the ticket if the user leaves your server. |
| Send inactivity notification? | If enabled, your bot will send a warning to the user before automatically closing the ticket, so they have a chance to respond. |
...
| Notification period | *Only available if “Send inactivity notification?” is enabled.*<br/>Set how long the user must be inactive before the warning is sent. Learn the SCNX duration format [here](/docs/custom-bot/additional-features/#durations). |
| Automatically close inactive tickets | If enabled, your bot will automatically close the ticket after user inactivity. |
| Time window for automatically closing tickets | *Only available if “Automatically close inactive tickets” is enabled.*<br/>Set how long the user must be inactive before the ticket is closed automatically. Learn the SCNX duration format [here](/docs/custom-bot/additional-features/#durations). |
...
| Field | Description |
| --- | --- |
| Reminder channel | Your bot will send reminders to this channel. |
| Remind the last (or assigned) team member? | If enabled, your bot will send a reminder message for the responsible team member. |
| Last team member reminder period | *Only available if “Remind the last (or assigned) team member?” is enabled.*<br/>Set how long the team member must be inactive before a reminder is sent. Learn the SCNX duration format [here](/docs/custom-bot/additional-features/#durations). |
| Reminder message for the last team member | *Only available if “Remind the last (or assigned) team member?” is enabled.*<br/>Customize the reminder message that will be sent. |
| Mention again if a ticket hasn’t received a team member response yet | If enabled, your bot will send a reminder message about tickets that haven’t been handled yet. |
| Period before reminding about an unassigned/unanswered ticket | *Only available if “Mention again if a ticket hasn’t received a team member response yet” is enabled.*<br/>Set how long tickets must remain unhandled before a reminder is sent. Learn the SCNX duration format [here](/docs/custom-bot/additional-features/#durations). |
| Unassigned ticket reminder message | *Only available if “Mention again if a ticket hasn’t received a team member response yet” is enabled.*<br/>Customize the reminder message that will be sent for unhandled tickets. |
...
* When a ticket is on hold, [Auto close tickets](#auto-close-tickets) is disabled for that ticket.
...
| Field | Description |
| --- | --- |
| Enable ticket hold command? | If enabled, ticket hold is active in your ticket system. |
| Lock ticket when it is put on hold? | If enabled, the ticket channel will be locked automatically as soon as the ticket is put on hold. |
...
| Field | Description |
| --- | --- |
| Ticket channel name format | This format is used by your bot to create names for new ticket channels. You can set different formats for different ticket topics on the [Ticket topics](https://scnx.app/glink?page=support-system/ticket-system/ticket-topics) page in your dashboard. More info [here](/docs/support-bot/ticket-system/ticket-topics). |
| Error message: user already has an open ticket | *Only sent if [configured](#main-configuration-configuration).*<br/>This message is sent when a user tries to open another ticket while they already have an active one. |
| Error: ticket not found | This message is sent when a team member tries to [claim](/docs/support-bot/ticket-system/claiming) a ticket that has already been closed or deleted. |
| Error: command used outside ticket channels | This message is sent when a team member runs ticket commands outside a ticket channel. |
| Ticket-by-command confirmation message | This message is sent when a user tries to open a new ticket via `/ticket open` ([if configured](#main-configuration-configuration)). |
| Error: insufficient permissions to add users to a ticket | This message is sent when a user without the required permissions tries to add/remove a user to/from a ticket ([if configured](#additional-ticket-members-configuration)). |
...
* Display different statistics in different channels to provide information about your ticket system.
