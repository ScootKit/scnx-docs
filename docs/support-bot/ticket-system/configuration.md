...
| Command | Description |
| --- | --- |
| ` /ticket admin-open ` | *Only available if “Allow staff to open tickets on behalf of a user?” is <a href="#main-configuration-configuration">enabled</a>.*<br/>Allows team members to create new tickets with specific users. Restrictions like multiple open tickets or opening hours are bypassed. |
...
| Allow users to open tickets via command? | If enabled, your users can create tickets not only via buttons and dropdowns, but also with <a href="#main-configuration-commands"><code>/ticket open</code></a>. |
| Allow staff to open tickets on behalf of a user? | If enabled, team members can create a ticket with a user using <a href="#main-configuration-commands"><code>/ticket admin-open</code></a>. Restrictions like multiple open tickets or opening hours are bypassed. |
| Category for new tickets | New tickets are created in this category by default. You can set different categories for different ticket topics on the [Ticket topics](https://scnx.app/glink?page=support-system/ticket-system/ticket-topics) page in your dashboard. More info [here](./ticket-topics.md). |
| Team member roles | These roles get access to tickets by default and are mentioned when a new ticket is created. You can set different roles for different ticket topics on the [Ticket topics](https://scnx.app/glink?page=support-system/ticket-system/ticket-topics) page in your dashboard. More info [here](./ticket-topics.md). |
...
| Ticket message | Your bot will send this message by default when a user creates a new ticket. You can set different messages for different ticket topics on the [Ticket topics](https://scnx.app/glink?page=support-system/ticket-system/ticket-topics) page in your dashboard. More info [here](./ticket-topics.md). |
| Allow only one ticket per user (global) | If enabled, a user can have at most one ticket open at a time. You can set different ticket limits per topic on the [Ticket topics](https://scnx.app/glink?page=support-system/ticket-system/ticket-topics) page in your dashboard. More info [here](./ticket-topics.md). |
...
| Allow team members to rename tickets? | If enabled, team members can rename tickets with <a href="#main-configuration-commands"><code>/ticket rename</code></a>. Note: after a manual rename, the bot can no longer automatically change the ticket name when transferring it (if enabled in [ticket topics](./ticket-topics.md)). |
| Show rename button under the ticket message? | *Only available if “Allow team members to rename tickets?” is enabled.*<br/>If enabled, a button is added under the ticket message so team members can rename the ticket faster. Otherwise they must use <a href="#main-configuration-commands"><code>/ticket rename</code></a>. |
...
| Command                             | Description |
|-------------------------------------| --- |
| ` /ticket close [force:<Boolean>] ` | *Only available if “Allow team members to close tickets without confirmation?” is <a href="#ticket-closing-configuration">enabled</a>.*<br/>Allows team members to close tickets even if a close request is open by setting the `force` option to `True`. |
...
| Field | Description |
| --- | --- |
| Show ticket close button in the ticket message? | If enabled, a button is added to the ticket message that team members (or users, if enabled) can use to close the ticket. Otherwise they must use <a href="#ticket-closing-commands"><code>/ticket close</code></a>. |
...
| Allow team members to close tickets without confirmation? | *Only available if “Require user confirmation before the ticket is closed?” is enabled.*<br/>If enabled, team members can close a ticket despite an open close request using <a href="#ticket-closing-commands"><code>/ticket close [force:&lt;Boolean&gt;]</code></a>. |
| Can team members manually schedule when tickets should be closed? | If enabled, team members can schedule closing tickets with <a href="#ticket-closing-commands"><code>/ticket schedule-close</code></a>. |
...
| Notification period | *Only available if “Send inactivity notification?” is enabled.*<br/>Set how long the user must be inactive before the warning is sent. Learn the SCNX duration format <a href="../../custom-bot/additional-features.md#durations">here</a>. |
| Automatically close inactive tickets | If enabled, your bot will automatically close the ticket after user inactivity. |
| Time window for automatically closing tickets | *Only available if “Automatically close inactive tickets” is enabled.*<br/>Set how long the user must be inactive before the ticket is closed automatically. Learn the SCNX duration format <a href="../../custom-bot/additional-features.md#durations">here</a>. |
...
| Last team member reminder period | *Only available if “Remind the last (or assigned) team member?” is enabled.*<br/>Set how long the team member must be inactive before a reminder is sent. Learn the SCNX duration format <a href="../../custom-bot/additional-features.md#durations">here</a>. |
...
* When a ticket is on hold, <a href="#auto-close-tickets">Auto close tickets</a> is disabled for that ticket.
