...
Rock paper scissors is a game that can be easily played with just two players and requires no additional equipment. If you want to learn more about the game and its rules, you can check out [Wikipedia](https://en.wikipedia.org/wiki/Rock_paper_scissors).
...
If the user option of the command is left empty or a bot is provided, the game starts automatically in the current channel against the Custom-Bot's AI.
