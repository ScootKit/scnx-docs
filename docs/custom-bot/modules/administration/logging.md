...
- Log role updates (like role edits, permission changes and more) in any channel you'd like. Member role changes (a role being added to or removed from a member) are also logged here, alongside changes to the role itself.
- Log Member-Updates (like nickname changes, member joins and leaves, and more) in any channel you'd like. Leave entries show how long the member was on the server and which roles they had when they left; kicks and bans are excluded to avoid double-logging. Role-add/remove changes are logged in the Role-Updates channel instead.
