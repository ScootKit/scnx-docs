...
| **Change nickname**                 | Change the nickname of a user. Leave blank to reset. When the [Role-Nicknames](/docs/custom-bot/modules/community/nicknames) module is enabled, the chosen name is stored as the user's preferred base name -- it is preserved across role changes, streak updates, and other automatic nickname updates instead of being overwritten by the next event. With the module disabled, the value is set on Discord directly and the bot does not retain it. |
