...
A V4 message is a JSON object with two fields: `_schema` (always `"v4"`) and `components` (an array of components that
make up the message).
...
The total component count across the entire message (including all children, accessories, and gallery items) must not
exceed **40**.
...
- `## Heading` renders as a heading (useful for titles inside Containers)
- `-# Small text` renders as small/muted text (useful for footers inside Containers)
- `**bold**`, `*italic*`, `~~strikethrough~~`, `` `code` `` all work
- `[link text](https://example.com)` renders as a clickable link
...
Places text alongside a small accessory (a thumbnail image or a button) on the right side. Great for profile-style
layouts or call-to-action blocks.
...
```json
{
  ...
  "components": [
    ...
    {
      "type": 10,
      "content": "Building tools for Discord communities."
    }
  ],
  "accessory": {
    "type": 11,
...
**Tip**: To convert a hex color to the integer value, use `parseInt("5865F2", 16)` which gives `5793266`. Some common
colors: blurple = `5793266`, green = `5763719`, red = `15548997`, yellow = `16705372`.
...
| `url`         | string  | Link buttons only  | Target URL (must be `http://` or `https://`). Max 512 characters.                                              |
...
```json
{
  ...
  "scnx_action": {
    "type": "customCommandElement"
  },
  "options": [
    {
      "label": "Show Stats",
      "value": "stats-btn-id",
      "description": "View your server stats",
      "emoji": "📊"
    },
    {
      "label": "Daily Reward",
...
| `emoji`       | string | No       | Unicode emoji character or custom emoji in `<:name:id>` format.                                    |

---

## Shared Content Notes {#shared-content-notes}

When creating shared content through the [Shared Marketplace Content API](/docs/scnx-api/marketplace/shared-content/), keep in
mind that your message will be imported by users on **different servers**. Components that reference guild-specific IDs
will not work:
  ...
  "components": [
    {
      ...
      "components": [
        {
          "type": 10,
          "content": "## 📢 Server Update v2.5"
        },
        {
          "type": 10,
          "content": "We've added new channels, updated the rules, and launched a giveaway! Check it out below."
        },
        {
          "type": 14,
          "divider": true,
        ...
        {
          "type": 10,
          "content": "-# Posted by the admin team • February 2026"
        }
      ]
    },
    {
      ...
      "components": [
        {
          ...
          "components": [
            ...
            {
              "type": 10,
              "content": "We're glad you're here. Take a look around, introduce yourself, and have fun."
            }
          ],
          "accessory": {
            "type": 11,
  ...
  "components": [
    {
      ...
      "components": [
        ...
        {
          "type": 10,
          "content": "The server will be under maintenance on **March 1st** from 2:00 to 4:00 PM UTC."
        }
      ]
    },
    {
      ...
      "components": [
        ...
        {
          "type": 10,
          "content": "- New #art-showcase channel\n- Updated self-role menu\n- Bot commands revamp"
        }
      ]
    },
    {
