...
- Erlaube Abstimmenden optional, mehr als eine Option pro Umfrage auszuwählen – pro Umfrage konfigurierbar über das Argument `max-selections`.
...
| Befehl                                                                                                                                            | Beschreibung                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/polls create description:<Text> channel:<TextKanal> option1:<Text> option2:<Text> [option{n}:<Text>] [duration:<ZeitDauer>] [public:<Boolean>] [max-selections:<Integer>]` | Erstelle im gewünschten Kanal eine neue Umfrage. Gib die Umfragefrage als `description` Wert ein und füge mögliche Optionen mit dem Element `option{n}` hinzu. Optional kannst Du eine Umfrage mit der Option `public` öffentlich machen (das bedeutet, dass jeder sehen kann, wofür Benutzer gestimmt haben). Wenn diese Option aktiviert ist, wird unterhalb der Umfrage die Schaltfläche „Aktuelle Stimmen anzeigen" hinzugefügt. Um eine Umfrage automatisch zu beenden, gibst du eine `duration` in einem [unterstützten Zeitdauerwert](/docs/custom-bot/additional-features#durations) an. Die Option `max-selections` steuert, wie viele Optionen jeder Abstimmende auswählen darf: `1` (Standard) ist eine klassische Einzelauswahl-Umfrage, jeder Wert über `1` erlaubt es Abstimmenden, bis zu so viele Optionen auszuwählen, und `0` bedeutet unbegrenzt (Abstimmende können bei Bedarf jede Option auswählen). |
