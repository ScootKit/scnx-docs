...
| Nachrichten-Drop-Nachricht       | Ein Array benutzerdefinierter Nachrichten, die gesendet werden, wenn ein Nutzer einen Nachrichten-Drop erhält - für jeden Drop wird zufällig eine ausgewählt. Unterstützt vollständige Embed-Formatierung und den Platzhalter `%earned%` (der verdiente Betrag inklusive Währungssymbol). Leer lassen, um auf die integrierte Standardnachricht zurückzugreifen. |
