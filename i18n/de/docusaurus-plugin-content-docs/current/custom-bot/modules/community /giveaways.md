...
| Multiple entries                  | Für jede konfigurierte Rolle hier (die ein Nutzer hat) wird zusätzliche . By default, every user has one entry. For every one of their roles configured here, they can recive additional entries, depending on this configuration.<br/><ul><li>First field: Role ID</li><li>Amount of *additional** entries members with this role will receive.</li></ul>                                                                                                                                                                      |
